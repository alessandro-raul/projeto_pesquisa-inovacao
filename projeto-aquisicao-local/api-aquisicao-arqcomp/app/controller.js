...
router.get("/sendData", (request, response) => {
  ...
  db.conectar()
    .then(() => {
      console.log("TES", gerarTemperatura() - (Math.random() + 1))
      const sql = `
        INSERT into dbo.dados (logDados, temperatura, umidade, fkSensor)
        values ('${agora()}', ${Math.random()*6+2}, ${parseInt(Math.random() * 40 + 20)}, 22);
        INSERT into dbo.dados (logDados, temperatura, umidade, fkSensor)
        values ('${agora()}', ${Math.random()*6+2}, ${parseInt(Math.random() * 40 + 20)}, 23);
        INSERT into dbo.dados (logDados, temperatura, umidade, fkSensor)
        values ('${agora()}', ${Math.random()*6+2}, ${parseInt(Math.random() * 40 + 20)}, 24);
        INSERT into dbo.dados (logDados, temperatura, umidade, fkSensor)
        values ('${agora()}', ${Math.random()*6+2}, ${parseInt(Math.random() * 40 + 20)}, 25);`;
      console.log(sql);
...
function gerarTemperatura() {
  return ArduinoDataTemp.List[ArduinoDataTemp.List.length - 1];

}

function gerarHumidade() {

  return ArduinoDataHumidity.List[ArduinoDataHumidity.List.length - 1];
}
